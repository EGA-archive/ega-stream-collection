...
public class EgaCipher {
    ...
    public void run(char[] password) {
        if (!this.gpg) {
            ...
            if (this.in != null) { // Input file(s) provided to read from
                boolean multi = false;
                if (this.in.length > 1) {
                    multi = true; // count num file completion (instead if single file progress)
                }
    ...
    public static void main(String[] args) {
        if (args.length == 0) { // No parameters -- print usage instructions
            ...
            System.out.println("java -jar EgaCipher.jar --test password file");
        ...
        EgaCipher t = null;
        try {
            t = new EgaCipher(threads, block, mod, gpg, pw_strength, files);
    ...
    private static void test(char[] pass, String file) {
        ...
        try {
            ...
            sct = new EgaSeekableCipherStream(new SeekableFileStream(new File(file + ".cip")), pass);
        } catch (FileNotFoundException ex) {
            Logger.getLogger(EgaCipher.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private static void xtest(char[] pass, String file) {
        ...
        try {
            ...
            EgaSeekableCipherStream sct = null; // Decrypted File
            sft = new SeekableFileStream(new File(file));
            sct = new EgaSeekableCipherStream(new SeekableFileStream(new File(file + ".xtest.cip")), pass);
