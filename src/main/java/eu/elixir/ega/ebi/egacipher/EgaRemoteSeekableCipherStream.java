 ...
 * @author asenf
 */
public class EgaRemoteSeekableCipherStream extends SeekableStream {

    public static final int DEFAULT_BUFFER_SIZE = 512000;

    final SeekableStream wrappedStream;
    Cipher cipher;
    EgaCipherInputStream incipher;
    ...
    // Constructors
    public EgaRemoteSeekableCipherStream(SeekableStream in, char[] password) {
        this(in, password, DEFAULT_BUFFER_SIZE);
    }

    public EgaRemoteSeekableCipherStream(SeekableStream in, char[] password, int bufferSize) {
        this(in, password, bufferSize, 256);
    }

    public EgaRemoteSeekableCipherStream(SeekableStream in, char[] password, int bufferSize, int pw_strength) {
        ...
        this.incipher = new EgaCipherInputStream(this.wrappedStream, cipher, bufferSize);
    ...
    public void seek(long position) throws IOException {
        ...
        try {
            ...
            Logger.getLogger(EgaRemoteSeekableCipherStream.class.getName()).log(Level.SEVERE, null, ex);
        }
        this.incipher = new EgaCipherInputStream(this.wrappedStream, cipher, bufferSize);
        this.wrappedCipher = new DataInputStream(this.incipher);
    }
