 ...
 */
package eu.elixir.ega.ebi.egacipher.experimental;
 ...
 * @author asenf
 */
public class EgaAsyncBufferedSeekableHTTPStream extends SeekableStream {
    ...
    public EgaAsyncBufferedSeekableHTTPStream(final URL url) {
        this(url, null, null);
    }

    public EgaAsyncBufferedSeekableHTTPStream(final URL url, String auth) {
        this(url, null, auth);
    }

    public EgaAsyncBufferedSeekableHTTPStream(final URL url, Proxy proxy) {
        this(url, proxy, null);
    }
    
    public EgaAsyncBufferedSeekableHTTPStream(final URL url, Proxy proxy, String auth) {
        this(url, proxy, auth, -1);
    }
    
    public EgaAsyncBufferedSeekableHTTPStream(final URL url, Proxy proxy, String auth, long fileSize) {
...
//            ListenableFuture<Response> execute = defaultAsyncHttpClient.prepareGet(url)
//                    .addHeader("Authorization", basicAuth).execute();
//            while (!execute.isDone()) {
//                Thread.sleep(1000);
//            }
//            Response get = execute.get();
//            byte[] responseBodyAsBytes = get.getResponseBodyAsBytes();
